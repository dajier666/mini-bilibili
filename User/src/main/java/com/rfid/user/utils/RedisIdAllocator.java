...
public class RedisIdAllocator {
    private static final Logger logger = LoggerFactory.getLogger(RedisIdAllocator.class);
    ...
    public long allocateWorkerId() {
        ...
        try {
            ...
            if (lock.tryLock(10, 30, TimeUnit.SECONDS)) {
                ...
                logger.info("Allocated worker ID: {}", nextId);
                return nextId;
            } else {
                logger.error("Failed to acquire lock for worker ID allocation");
                throw new RuntimeException("Failed to allocate worker ID");
            }
        } catch (Exception e) {
            logger.error("Error allocating worker ID", e);
    ...
    public long allocateDataCenterId() {
        ...
        try {
            if (lock.tryLock(10, 30, TimeUnit.SECONDS)) {
                ...
                logger.info("Allocated datacenter ID: {}", nextId);
                return nextId;
            } else {
                logger.error("Failed to acquire lock for datacenter ID allocation");
                throw new RuntimeException("Failed to allocate datacenter ID");
            }
        } catch (Exception e) {
            logger.error("Error allocating datacenter ID", e);
